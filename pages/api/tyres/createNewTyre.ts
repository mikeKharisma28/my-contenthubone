import { CreateNewTyre } from '@/lib/tyres/tyres-lib';
...
export default async function handler(_request: ContentHubParams, response: NextApiResponse) {
  ...
  try {
    ...
    const { name, type, price, width, profile, rimSize } = _request.body;
    const data = await CreateNewTyre(name, type, price, width, profile, rimSize);
